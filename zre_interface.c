...
zre_interface_t *
zre_interface_new (char ** peer_address)
{
    zre_interface_t
        *self;
    self = (zre_interface_t *) zmalloc (sizeof (zre_interface_t));
    self->ctx = zctx_new ();
	// Creates a new zmq context that'll be used throughout the program
	
    self->pipe = zthread_fork (self->ctx, zre_interface_agent, peer_address);
	//zthread_fork returns a child socket acting as the opposite end of the pipe. Attached threads can talk to parent threads over the pipe
...
zre_interface_join (zre_interface_t *self, const char *group)
{
	...
	// Send a string to a socket with more flags

    zstr_send  (self->pipe, group);
	// Send a string to a socket in ZMQ string format
...
zre_interface_leave (zre_interface_t *self, const char *group)
{
	...
	//Send a string to a socket with more flag
...
//	Add a port number for the client over here 

    self->port = zsocket_bind (self->inbox, "tcp://%s:*", self->host);
	printf("SELF IP is %s and SELF PORT is %d\n", self->host, self->port);
//      self->port = zsocket_bind (self->inbox, "tcp://%s:*", peer_address);
//	For testing purpose changing the interface to loopback
...
//	printf("Sending Message to IP %s and port %ld\n", self->udp, (long)self->port);
...
static int
agent_recv_udp_beacon (agent_t *self, char **peer_address)
{
...
//    if (memcmp (beacon.uuid, self->uuid, sizeof (uuid_t))) {
	...
	printf("IP is %s Port is %s\n", peer_address[0], peer_address[1]);
//	printf("IP is %s\n", (char *) zre_udp_from(self->udp));
        zre_peer_t *peer = s_require_peer (
//            self, identity, zre_udp_from (self->udp), 49152);
            self, identity, peer_address[0], atoi(peer_address[1]));
        zre_peer_refresh (peer);
        free (identity);
...
//	printf("Peer Address is %s\n", (char *)args);
    ...
    char **peer_address = args; 
    //  Send first beacon immediately

//	Send Connect Request

                if(peer_address != NULL) {
                        agent_recv_udp_beacon (self, peer_address);
                }
